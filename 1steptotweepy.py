...
#client.retweet('1556267441650540551', user_auth=False)
    for ind in range(11):
        ...
        search = client.search_recent_tweets(requete,max_results=50,since_id=dernier_id) #since_id=dernier_id
        #print(search.json())
        search_json = search.json()
        if search_json['meta']['result_count'] != 0:
            print('Data found '+ str(search_json['meta']['result_count'])+ " Tweets pour " + nom_tip)
        ...
        #g = open("test.txt","w",encoding="utf-8")
            ...
            print('No data found pour ' + nom_tip)
    
print('data_import ended')
